...
public final class Debugger implements VideoReciever {
     ...
     * 
     * @param path ROM-file to load into the CPU.
     */
    ...
    private void scanCommand(final Scanner in) {
        ...
        if (scLine.equals("t")) {
            ...
            if (jfr == null) {
                ...
                panelScreen.setPreferredSize(new Dimension(zoom * LCD_W, zoom
                        * LCD_H));
                jfr.setContentPane(panelScreen);
                jfr.addKeyListener(new SimpleKeyInputController(addressBus
                        .getJoyPad()));
                jfr.pack();
            }
            ...
            // Save State
        } else if (scLine.equals("savestate")) {
            try {
                System.out.println("Testing state load/save...");
            ...
            // Read State
        } else if (scLine.equals("loadstate")) {
            try {
    ...
    private void spriteTable() {
        for (int i = 0; i < GB_SPRITE_ATTRIBUTES; i++) {
            ...
            System.out.println(String.format(
                    "%03d : %03d, id: %02x, flags: %02x", x, y, id, fl));
        }
    }
    ...
    private void toggleKey(String key) {
         ...
         * (key.equals("select")) { jp.setSelect(!jp.isSelect()); } else if
         * (key.equals("up")) { jp.setUp(!jp.isUp()); } else if
         * (key.equals("down")) { jp.setDown(!jp.isDown()); } else if
         * (key.equals("left")) { jp.setLeft(!jp.isLeft()); } else if
         * (key.equals("right")) { jp.setRight(!jp.isRight()); } else {
     ...
     * 
     * @param addr Where to dump memory.
     */
     ...
     * 
     * @param addr Address to start writing out, rounded down.
     * @param len Number of bytes to dump, rounded up.
     */
     ...
     * @param toWrite A list of bytes to write.
     */
     ...
     * 
     * @param reg Register to write set value on.
     ...
     * 
     * @param addr Address to toggle breakpoint at.
     */
     ...
     * 
     * @param addr Address to check breakpoint on.
     ...
     * @throws InterruptedStepException if the interruption was interrupted,
     *             this is thrown.
     */
     ...
     *            stepped forever.
     * @param steps If forever is set to fals, debugNSteps will step steps
     *            steps.
     */
    ...
    private void printDirtyActions() {
        for (DebuggerAddressBus.MemoryAction ma : addressBus.getDirtyActions()) {
